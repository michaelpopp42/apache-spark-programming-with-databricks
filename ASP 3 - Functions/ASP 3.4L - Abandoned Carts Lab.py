...
carts_df = (events_df
            .withColumn("itmes",explode("items"))
            .groupBy("user_id")
            .agg(array_distinct(flatten(collect_set("items.item_id"))).alias("cart"))
)
display(carts_df)
...
abandoned_carts_df = (email_carts_df
                      .where((col("converted") == lit(False)) & col("cart").isNotNull())
)
